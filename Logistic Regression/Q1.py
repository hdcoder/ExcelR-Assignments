...
print(Bank_Data_1["Target"].value_counts())
print()

No_sub = len(Bank_Data_1[Bank_Data_1["Target"] == "no"])
Yes_sub = len(Bank_Data_1[Bank_Data_1["Target"] == "yes"])
print(No_sub, Yes_sub)
print((Yes_sub/(Yes_sub + No_sub)) * 100)
print()

plt.figure()
pd.crosstab(Bank_Data_1["job"], Bank_Data_1["Target"]).plot(kind = "bar")
plt.title("Crosstab")
plt.xlabel("Job")
plt.ylabel("Frequency of Subscription")

plt.figure()
Fig = pd.crosstab(Bank_Data_1.marital, Bank_Data_1.Target)
...
plt.figure()
Fig = pd.crosstab(Bank_Data_1.education, Bank_Data_1.Target)
Fig.div(Fig.sum(1).astype(float),axis=0).plot(kind='bar',stacked=True)
...
plt.figure()
Fig = pd.crosstab(Bank_Data_1.contact, Bank_Data_1.Target)
Fig.div(Fig.sum(1).astype(float),axis=0).plot(kind='bar',stacked=True)
...
plt.figure()
Fig = pd.crosstab(Bank_Data_1.poutcome, Bank_Data_1.Target)
Fig.div(Fig.sum(1).astype(float),axis=0).plot(kind='bar',stacked=True)
...
plt.figure()
pd.crosstab(Bank_Data_1.month, Bank_Data_1.Target).plot(kind='bar')
...
Bank_Data_1['Target'] = Bank_Data_1['Target'].map({'yes':1,'no':0})
...
X = Bank_Data_1.loc[:, Bank_Data_1.columns != "Target"]
Y = Bank_Data_1.loc[:, Bank_Data_1.columns == "Target"]
